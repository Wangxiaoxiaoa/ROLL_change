                ...
                for retry in range(max_retries):
                    try:
                        ...
                        if retry < max_retries - 1:
                            import time
                            time.sleep(1) # 简单休眠后重试
                            continue
                        raise e
        except Exception as e:
            if isinstance(image, bytes):
                image_out = [Image.new("RGB", (224, 224), (255, 255, 255))]
                logger.error(f"Failed to get image with type: {type(image)}, error: {e}")
            else:
                image_out = [Image.new("RGB", (224, 224), (255, 255, 255))] * len(image)
                logger.error(f"Failed to get image: {image} after {max_retries} retries, error: {e}")
